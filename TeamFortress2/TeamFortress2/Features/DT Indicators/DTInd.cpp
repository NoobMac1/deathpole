...
void DTInd::DrawInd()
{
	...
	if (g_Menu.m_bOpen || g_Menu.m_flFadeElapsed < g_Menu.m_flFadeDuration)
	{
		...
		g_Draw.Rect(m_nIndX - m_nRadarSize, m_nIndY - m_nRadarSize - 10, m_nRadarSize * 2, 10, Vars::Menu::Colors::TitleBar);
	...
	//Background
	g_Draw.Rect(m_nIndX - m_nRadarSize, m_nIndY - m_nRadarSize, m_nRadarCorrSize, m_nRadarCorrSize, clrBack);

	//Outline
	g_Draw.OutlinedRect(m_nIndX - m_nRadarSize, m_nIndY - m_nRadarSize, m_nRadarCorrSize, m_nRadarCorrSize, Vars::Menu::Colors::TitleBar);

	//Center lines
	g_Draw.Line(m_nIndX, m_nIndY - m_nRadarSize, m_nIndX, m_nIndY + m_nRadarSize - 1, Vars::Menu::Colors::TitleBar);
	g_Draw.Line(m_nIndX - m_nRadarSize, m_nIndY, m_nIndX + m_nRadarSize - 1, m_nIndY, Vars::Menu::Colors::TitleBar);
}

bool DTInd::GetDrawPosition(int& x, int& y, CBaseEntity* pEntity)
{
	...
	x = (m_nIndX + static_cast<int>(vPos.x / m_flRange * m_nRadarSize));
	y = (m_nIndY + static_cast<int>(vPos.y / m_flRange * m_nRadarSize));

	//Just confirm that they were both set.
	return (x != -1 && y != -1);
}

void DTInd::DrawPoints(CBaseEntity* pLocal)
{
...
void DTInd::DragInd()
{
	...
	if ((mousex > (m_nIndX - m_nIndSize) &&
		mousex < (m_nIndX - m_nIndSize) + (m_nIndSize * 2) &&
		mousey >(m_nIndY - m_nIndSize) - 10 &&
		mousey < (m_nIndY - m_nIndSize)) && bHeld)
	{
		m_bDrag = true;
		
		if (!m_bMove) 
		{
			pCorrect.x = mousex - m_nIndX;
			pCorrect.y = mousey - (m_nIndY - m_nIndSize);
			m_bMove = true;
		}
	}
	...
	if (m_bDrag)
	{
		m_nIndX = (mousex + m_nIndSize) - (m_nIndSize) - pCorrect.x;
		m_nIndY = (mousey + m_nIndSize) - pCorrect.y;
	}

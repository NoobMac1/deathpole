...
std::string GetSpam(const int nIndex) {
	...
	switch (nIndex)
	{
	case 0: str = XorStr("say deathpole - best private hack").str(); break;
	case 1: str = XorStr("say deathpole - private exploits galore").str(); break;
	case 2: str = XorStr("say deathpole - crashing servers since 2016").str(); break;
	case 3: str = XorStr("say deathpole - the most satanic cheat").str(); break;
	default: str = XorStr("say deathpole - deathpole shitting on ur p2c").str(); break;
	}
...
void CMisc::SteamRPC()
{
	...
	*/
	...
	{
	case 0:
		g_SteamInterfaces.Friends015->SetRichPresence("currentmap", "Deathpole");
		break;
	case 1:
...
void Notify::Think() {
	...
	for (size_t i{}; i < m_notify_text.size(); ++i) {
		...
		*/ //obsolete
		...
		g_Draw.String(FONT_INDICATORS, x + 6, y +2, { Colors::DmgLoggerText.r, Colors::DmgLoggerText.g, Colors::DmgLoggerText.b, color.a }, ALIGN_DEFAULT, notify->m_text.c_str());

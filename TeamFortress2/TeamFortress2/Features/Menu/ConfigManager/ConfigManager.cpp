...
{
	m_sConfigPath = std::filesystem::current_path().wstring() + _(L"\\DeathPole");
	...
	if (!std::filesystem::exists(m_sConfigPath + _(L"\\DeathCore")))
		std::filesystem::create_directory(m_sConfigPath + _(L"\\DeathCore"));
}

void CConfigManager::Save(const wchar_t *name)
{
	m_Write = std::wofstream(m_sConfigPath + L"\\" + name + _(L".dp"));
	...
	if (m_Write.is_open())
	{
		...
		//Misc
		{
			...
			// CL_Move
			{
				...
				SAVE_VAR(Vars::Misc::CL_Move::FakelagValueMin);// { 0x52, L"Recharge Key" }; //R
				SAVE_VAR(Vars::Misc::CL_Move::FakelagValueMax);// { 0x52, L"Recharge Key" }; //R
			}
			//Discord
			{
		...
		//AntiHack
		{
			//AntiAim
			{
				...
				SAVE_VAR(Vars::AntiHack::AntiAim::RollStep);
				SAVE_VAR(Vars::AntiHack::AntiAim::CustomPitch);
			}
			//Resolver
			{
...
{
	m_Read = std::wifstream(m_sConfigPath + L"\\" + name + _(L".dp"));
	...
	if (m_Read.is_open())
	{
		...
		//Misc
		{
			...
			// CL_Move
			{
				...
				LOAD_VAR(Vars::Misc::CL_Move::FakelagValueMin);// { 0x52, L"Recharge Key" }; //R
				LOAD_VAR(Vars::Misc::CL_Move::FakelagValueMax);// { 0x52, L"Recharge Key" }; //R
			}
			//Discord
			{
		...
		//AntiHack
		{
			//AntiAim
			{
				...
				LOAD_VAR(Vars::AntiHack::AntiAim::RollStep);
				LOAD_VAR(Vars::AntiHack::AntiAim::CustomPitch);
			}
			//Resolver
			{
...
{
	std::filesystem::remove(m_sConfigPath + L"\\" + name + _(L".dp"));
}
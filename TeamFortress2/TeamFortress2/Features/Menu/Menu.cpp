...
void CMenu::Render(IDirect3DDevice9* pDevice) {
	...
	*/
	if (g_Menu.m_bOpen)
	{
		...
		if (ImGui::Begin("deathpole | debug | logged in as admin", nullptr, ImGuiWindowFlags_NoCollapse))
		{
			//ImGui::PopStyleColor();
					...
					ImGui::BeginChild("Visuals");
					{
						...
						ImGui::NextColumn();
						{
							...
							if (ImGui::CollapsingHeader("Miscellaneous")) {
								...
								ImGui::PushItemWidth(100); ImGui::SliderFloat("Prop opacity", &Vars::Visuals::PropAlpha.m_Var, 0.01f, 1.0f, "%.2f"); ImGui::PopItemWidth(); HelpMarker("How opaque props are");
					...
					ImGui::BeginChild("Misc");
					{
						ImGui::Columns(3);
						{
							...
							if (ImGui::CollapsingHeader("Steam")) {
								...
								const char* mapTexts[]{ "Deathpole", "Figoraware", "Meowhook.club", "Rathook.cc", "Nitro.tf", "custom" }; ImGui::PushItemWidth(150); ImGui::Combo("Map text", &Vars::Misc::Steam::MapText.m_Var, mapTexts, IM_ARRAYSIZE(mapTexts)); ImGui::PopItemWidth(); HelpMarker("Which map text should be used?");
